...
async def hfmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        lel = await message.reply("`memperoses...`")
...
async def m_cb(b, cb):
    ...
    if type_ == "pause":
        ...
        now_playing = temp[0][0]
        by = temp[0][1].mention(style="md")
        msg = "**⚡️Sedang memutar** in {}".format(cb.message.chat.title)
        msg += "\n- " + now_playing
        msg += "\n-  " + by
        temp.pop(0)
        if temp:
...
async def play(_, message: Message):
    ...
    if audio:
        ...
        try:
            ...
            while j < 5:
                toxxt += f"{emojilist[j]} <b>Judul - [{results[j]['title']}](https://youtube.com{results[j]['url_suffix']})</b>\n"
                toxxt += f" ╚ ⏰<b>Durasi</b> - {results[j]['duration']}\n"
                toxxt += f" ╚ 👁️<b>Ditonton</b> - {results[j]['views']}\n"
...
async def ytplay(_, message: Message):
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="▶️ <b>Sedang memutar </b> Atas Permintaan {} via Youtube Music 😎".format(
                message.from_user.mention()
            ),
        )
        os.remove("final.png")
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        ...
        return
    await cb.message.edit("<i>Tunggu...\nMusik akan segera diputar 🚀</i>")
    x=int(x)
    try:
